...
export const uploadProfilePicture = async (req, res) => {
  ...
  // Construct public URL (adjust based on your deployment/env)
  // For local dev: http://localhost:5000/uploads/profile_pictures/filename
  const fileUrl = `${req.protocol}://${req.get("host")}/uploads/profile_pictures/${req.file.filename}`;

  try {
    // Update Postgres
    console.log(`[DEBUG] Updating user ${req.user.id} with photoURL: ${fileUrl}`);
    const updateResult = await pool.query("UPDATE users SET photo_url = $1 WHERE user_id = $2 RETURNING *", [
      fileUrl,
      req.user.id,
    ]);
    console.log("[DEBUG] Update result:", updateResult.rows[0]);
